...
## 六、B 站粉丝动态（≤233 字，发布视频时直接填）

> 这是 B 站投稿表单里"粉丝动态"那一栏，和上面的独立动态**不是同一个东西**。
> 这条会随视频发布**自动**推到所有粉丝的首页 feed，233 字硬限制。
> 写法上要"勾起来想看"而不是写简介；放具体钩子（金句 / 反直觉点 / 自嘲）。

```
<TODO: 主版本，约 130–180 字，结尾可以加 (=˚ω˚)/ 之类颜文字>
```

**备选短款**（约 60–100 字，余量更大）：
...
- [ ] 描述区粘贴章节列表，B 站和 YouTube 都会自动识别成 chapters
- [ ] 主封面用上面 prompt 生成；如平台压缩字号，手动加文字图层
- [ ] B 站补「视频简介」字段（复用第二节描述前两段）
- [ ] **B 站「粉丝动态」字段填第六节文案**（最容易忘）
- [ ] 视频号选话题（<TODO: 主题>）
- [ ] 发完同步发布独立动态 / Twitter / 公众号
"""
